Godot에서 **Input.get_axis()** 와 **Input.get_vector()** 는

둘 다 “입력 방향값”을 구하지만, **차원이 다르고 (1D vs 2D)**

**리턴값의 형태와 정규화 여부**가 완전히 다릅니다.
...
- > get_axis() → 1D용 (좌우 등 한 축)
    
- > get_vector() → 2D용 (상하좌우, 조이스틱 포함)
    
- > get_vector()는 대각선 입력 시 자동으로 정규화돼 속도 유지가 쉬워요.
...
> ⚙️ **“1D 움직임”과 “2D 움직임”의 목적이 다르기 때문에**,

> 실제 개발에서는 여전히 둘 다 쓰입니다.
...
## 1️⃣ 사실관계:get_vector() =get_axis() × 2 + normalize()
...
✅ get_vector()는 이미 get_axis() 두 번을 호출한 **상위 함수**가 맞습니다.

그래서 “결과만 필요하다면 get_vector로 통일 가능”해요.

---

## 2️⃣ 하지만 get_vector()는 2D 이동 전제 라서 주의점이 있습니다**
...
- get_vector()는 자동 정규화해서,
    
    X/Y를 모두 고려한 벡터로 만들어버립니다.
...
## 4️⃣ 반대로 “탑다운 2D”에서는 get_vector() 가 완전한 상위호환
...
## **⚙️ 5️⃣ 정리: “언제 get_vector만 써도 되는가?”**
...
## **🧩 6️⃣ “get_vector 하나로 통합하고 싶을 때” 팁**

  

만약 프로젝트 전체를 get_vector로 통일하고 싶다면,

“Y축 입력이 없는 경우”엔 그냥 무시하면 됩니다.
...
- > get_vector()는 get_axis()를 두 번 쓰는 상위 버전이 맞습니다.
    
- > 하지만 **1D 입력만 필요한 게임(플랫폼, 슬라이더)** 에선 get_axis()가 더 단순하고 안전해요.
    
- > 그 외 모든 2D 이동 로직이라면 get_vector() 하나로 통일해도 전혀 문제 없습니다.
